...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        self.attention_norm = nn.LayerNorm(dim, elementwise_affine=False, eps=norm_eps)
        self.ffn_norm = nn.LayerNorm(dim, elementwise_affine=False, eps=norm_eps)
...
class DiT_Llama(nn.Module):
    def __init__(
        ...
        self.cap_embedder = nn.Sequential(
            nn.LayerNorm(cap_feat_dim, elementwise_affine=False, eps=norm_eps),
            nn.Linear(
                cap_feat_dim, min(dim, 1024), bias=True,
            ),
        )
        for m in self.cap_embedder:
    ...
    def forward(self, x, t, caption):
        ...
        cap = self.cap_embedder(caption)  # (N, D)
        adaln_input = t.to(x.dtype) + cap.to(x.dtype)
...
def DiT_Llama_600M_patch8(**kwargs):
    return DiT_Llama(in_channels=16, patch_size=2, dim=512, n_layers=16, n_heads=32, **kwargs)


if __name__ == "__main__":
    model = DiT_Llama_600M_patch8()
    model.eval()
    print(f"Num parameters: {sum(p.numel() for p in model.parameters())}")
    x = torch.randn(2, 16, 32, 32)
    t = torch.randint(0, 100, (2,))
    cap = torch.randn(2, 768)
